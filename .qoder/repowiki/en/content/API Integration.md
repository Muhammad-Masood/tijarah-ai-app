...
## Introduction
This document explains the API integration patterns used in the application, focusing on the centralized API client, authentication and headers, marketplace-specific hooks for Shopify and Daraz, real-time updates via Server-Sent Events (SSE), error handling strategies, and guidance for adding new endpoints and maintaining backward compatibility.

## Project Structure
The API layer is centralized in a single module that provides typed functions for HTTP requests, SSE streaming, and marketplace integrations. Hooks encapsulate platform-specific flows (Shopify, Daraz) and compose them with authentication state to fetch products and manage connections.
...
- API client: low-level HTTP and SSE primitives.
- Hooks: stateful composition of auth, marketplace connections, and data fetching.
- UI: consumes hooks to render product lists and actions.

```mermaid
sequenceDiagram
participant UI as "UI Component"
participant Hook as "useShopifyProducts / useDarazProducts"
...
## Dependency Analysis
The following diagram shows how hooks depend on the API client and authentication context.
...
## Conclusion
The API integration layer provides a robust, typed, and extensible foundation for interacting with the backend and marketplace services. It standardizes error handling, supports real-time updates via SSE, and abstracts marketplace differences through normalization and hooks. By following the patterns outlined here, you can confidently add new endpoints, maintain backward compatibility, and deliver responsive user experiences across platforms.
