...
## Update Summary
**Changes Made**
- Updated documentation for Expo SDK 57 and React Native 0.86.3 compatibility
- Documented new NativeTabs API implementation replacing deprecated props
- Added comprehensive coverage of web-specific tab implementation using expo-router/ui
- Updated tab bar styling and theme handling for modern Expo Router APIs
- Enhanced documentation for improved navigation state management across app states
...
## Introduction
This document explains the navigation architecture of Tijarah AI App built with Expo Router SDK 57. It covers:
- File-based routing using Expo Router groups and stacks with modern API patterns
- Authentication-based navigation at the root level using Stack.Protected guards
- **Updated**: Modern tab-based navigation using NativeTabs API with enhanced theme handling
- Web-specific tab implementation using expo-router/ui components
...
- (app): Authenticated screens including a modern tab navigator, finance module, and feature stacks
...
TabsGroup --> NativeTabs["Native Tabs<br/>src/components/app-tabs.tsx"]
TabsGroup --> WebTabs["Web Tabs<br/>src/components/app-tabs.web.tsx"]
NativeTabs --> MoreTab["More Tab<br/>src/app/(app)/(tabs)/more.tsx"]
WebTabs --> MoreTab
...
- [src/components/app-tabs.tsx:7-65](file://src/components/app-tabs.tsx#L7-L65)
...
- Root layout and shell:
  - Provides theme provider and auth provider with modern Expo Router integration
...
- Auth context:
  - Hydrates session from a stored token using expo-secure-store
  - Exposes sign-in, sign-up, and sign-out functionality
...
- **Updated**: Modern tabs group:
  - Hosts NativeTabs for mobile platforms with enhanced theme handling
  - Uses web-specific Tab components for cross-platform consistency
  - Provides five destinations: Home, Insights, Ask Tijarah, Products, More
- Finance module:
...
- **Updated**: Modern tab system provides consistent experience across native and web platforms with improved theme handling
...
Inside (app), a nested Stack hosts a modern tabs group, several feature screens, and a comprehensive finance module. The tabs group renders platform-specific implementations with consistent behavior across native and web platforms.
...
participant Tabs as "Modern Tabs"
...
AppGroup->>Tabs : Mount NativeTabs/WebTabs
...
- [src/components/app-tabs.tsx:12-63](file://src/components/app-tabs.tsx#L12-L63)
- [src/components/app-tabs.web.tsx:26-49](file://src/components/app-tabs.web.tsx#L26-L49)
...
**Updated** The tab system has been modernized to use Expo Router SDK 57 APIs with platform-specific implementations:

- **Native Implementation** (`app-tabs.tsx`):
  - Uses `NativeTabs` from `expo-router/unstable-native-tabs`
  - Enhanced theme handling with proper color scheme detection
  - Improved icon rendering with `VectorIcon` component
  - Better accessibility and performance optimization

- **Web Implementation** (`app-tabs.web.tsx`):
  - Uses modern `Tabs`, `TabList`, `TabTrigger` components from `expo-router/ui`
  - Custom tab button implementations with raised center button
  - Consistent styling across platforms with theme-aware colors
  - Enhanced user interaction with press states and focus indicators
...
AppTabs --> NativeTabs : "mobile platform"
AppTabs --> WebTabs : "web platform"
```
...
- The (app) group defines a Stack with the modern tabs group, feature screens, and comprehensive finance module
- The tabs group renders platform-specific implementations with consistent behavior
...
- **Updated**: Modern tab system provides consistent navigation patterns across platforms
...
- Root layout depends on:
  - Theme provider for UI colors with modern Expo Router integration
...
- App and Auth groups depend on:
  ...
  - **Updated**: Modern tab system dependencies for cross-platform compatibility
  - Finance module dependencies for comprehensive financial analytics
- **Updated**: Modern tabs group depends on:
  - Platform-specific implementations (NativeTabs for mobile, Tabs for web)
  - Enhanced theme and color scheme handling
  - Cross-platform consistency utilities
...
TabsGroup --> NativeTabs["components/app-tabs.tsx"]
TabsGroup --> WebTabs["components/app-tabs.web.tsx"]
NativeTabs --> MoreTab["more.tsx"]
WebTabs --> MoreTab
...
- **Updated**: Modern tab system provides optimized performance with:
  - Platform-specific implementations for optimal rendering
  - Enhanced theme handling to minimize re-renders
  - Lazy loading patterns for large icon sets
  - Improved memory management for tab state
...
- **Updated**: Modern tab system issues:
  - Verify platform-specific tab implementations are properly configured
  - Check theme integration for both native and web platforms
  - Ensure proper icon rendering across different platforms
  - Validate tab state persistence and restoration
...
## Conclusion
Tijarah AI App uses a clean, scalable navigation architecture with modern Expo Router SDK 57 capabilities:
...
- **Updated**: Modern tab structure with platform-specific optimizations provides consistent experience across native and web platforms
- A nested tab structure organizes core features behind the authenticated boundary with enhanced theming and performance
- **Updated**: Cross-platform tab implementation using NativeTabs for mobile and modern Tabs components for web
...
- **Updated**: Enhanced theme handling and improved user experience across all platforms
- Splash and auth hydration are coordinated to provide a smooth initial experience

The modernization of the navigation system with Expo SDK 57 brings improved performance, better cross-platform consistency, and enhanced developer experience while maintaining backward compatibility and preserving the existing architectural patterns that make the app scalable and maintainable.