...
- (app): Authenticated screens including a tab navigator and feature stacks

At the root layout, a single Stack decides which group to mount based on the current session. Inside the authenticated group, a nested stack hosts a tabs group plus additional feature screens.
...
- [src/app/(app)/_layout.tsx:1-22](file://src/app/(app)/_layout.tsx#L1-L22)
...
- App group:
  - Declares authenticated routes: tabs, store flows, profile, notifications, product details, catalog detail
- Tabs group:
...
Inside (app), a nested Stack hosts a tabs group and several feature screens. The tabs group renders a native tab bar with five destinations.
...
Nav->>AppGroup : Mount (app) via Stack.Protected guard={!!session}
end
```
...
- The (app) group defines a Stack with the tabs group and feature screens
...
- [src/app/(app)/_layout.tsx:1-21](file://src/app/(app)/_layout.tsx#L1-L21)
- [src/app/(app)/(tabs)/_layout.tsx:1-7](file://src/app/(app)/(tabs)/_layout.tsx#L1-L7)
- [src/components/app-tabs.tsx:1-64](file://src/components/app-tabs.tsx#L1-L64)
