...
- Web-specific tab bar implementation versus native mobile tabs
...
## Project Structure
The app uses Expo Router with a root layout that gates navigation based on authentication state and shows an animated splash overlay until fonts and session are ready. The tab bar is implemented differently per platform:
- Native (iOS/Android): Uses native tabs from expo-router/unstable-native-tabs
- Web: Uses a custom web tab bar built on expo-router/ui Tabs
...
C --> D["Native Tabs<br/>src/components/app-tabs.tsx"]
C --> E["Web Tabs<br/>src/components/app-tabs.web.tsx"]
...
- [src/app/(app)/(tabs)/_layout.tsx:1-7](file://src/app/(app)/(tabs)/_layout.tsx#L1-L7)
...
- Tabs: native uses OS tab bar; web renders a floating pill-style bar with a raised center action.
...
The runtime flow coordinates splash, fonts, auth, and navigation while respecting platform capabilities.
...
Tabs-->>App : Render native or web tab bar
```
...
### Web Tabs vs Native Tabs
- Native tabs use OS-provided tab bars with selected colors and platform icons.
- Web tabs implement a floating pill container with a raised center “Ask Tijarah” button using CSS-like styles and pressable interactions.
- Both expose the same route names so navigation remains consistent across platforms.

```mermaid
flowchart TD
Start(["Mount Tab Layout"]) --> Detect{"Platform?"}
Detect --> |iOS/Android| Native["Render NativeTabs<br/>selectedColor from theme"]
Detect --> |Web| Web["Render Custom TabList<br/>Raised Ask Tijarah Button"]
...
- Prefer platform-native components where available (e.g., native tabs) to reduce custom layout costs.
...
- Using platform-specific implementations for tabs and splash
